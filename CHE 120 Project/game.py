#Sullivan[21205766], Aiden[21200435]
import pygame
import math
import random
import time
...
DEATH_SCREEN = pygame.transform.scale(
    pygame.image.load('death_screen.png'), (WIDTH, HEIGHT))
...
background = pygame.transform.scale(pygame.image.load(
    'game_background.png').convert_alpha(), (WIDTH, HEIGHT))
...
def main():
    ...
    class PLAYER(pygame.sprite.Sprite):
        def __init__(self):
            ...
            self.image = pygame.transform.scale(pygame.image.load(
                'e.png').convert_alpha(), (PLAYER_WIDTH, PLAYER_HEIGHT))
    ...
    class TITLE(pygame.sprite.Sprite):
        def __init__(self):
            super().__init__()
            self.image = pygame.transform.scale(pygame.image.load(
                'derivative_dodge_title_sprite.png').convert_alpha(), (TITLE_WIDTH, TITLE_HEIGHT))
            self.rect = self.image.get_rect()
        ...
        def update(self):
            ...
            self.rect.x = (WIDTH / 2 - TITLE_WIDTH / 2) + \
                5 * math.cos(self.angle)

    class OBSTACLE_RIGHT(pygame.sprite.Sprite):
        def __init__(self):
            super().__init__()
            self.image = pygame.transform.scale(pygame.image.load(
                'vector_obstacle_right.png').convert_alpha(), (OBSTACLE_WIDTH, OBSTACLE_HEIGHT))
            self.rect = self.image.get_rect()
            self.center_x = -550
            self.rect.y = -491
    ...
    class OBSTACLE_LEFT(pygame.sprite.Sprite):
        def __init__(self):
            super().__init__()
            self.image = pygame.transform.scale(pygame.image.load(
                'vector_obstacle_left.png').convert_alpha(), (OBSTACLE_WIDTH, OBSTACLE_HEIGHT))
            self.rect = self.image.get_rect()
            self.center_x = 750
            self.rect.y = -500
            self.rect = pygame.Rect(self.rect.x, self.rect.y, 1024, 50)
    ...
    class START_BUTTON(pygame.sprite.Sprite):
        def __init__(self):
            super().__init__()
            self.image = pygame.transform.scale(pygame.image.load(
                'press-space-to-start.png').convert_alpha(), (START_BUTTON_WIDTH / 4, START_BUTTON_HEIGHT / 4))
            self.rect = self.image.get_rect()
    ...
    while running:
        ...
        if not in_game:
            ...
            player.rect.y = 600
        if in_game == True:
            draw_after_title(background, player, obstacle_left,
                             obstacle_right, score_text, score_value)
        if in_game == False:
            draw_before_title(background, sine_sprites,
                              high_score_value, high_score_text)
    pygame.quit()
